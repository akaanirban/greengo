...
class GroupCommands(object):
	def __init__(self):
		...
		self._s3 = s.client("s3")				#-----------------Added Anirban
	...
	def create(self, full_pipeline_flag=False, reuse_lambda=False):
		...
		############################# ADDED BY ANIRBAN#############################################
		if full_pipeline_flag:
			# 7. Creating the required S3 buckets
			self.create_s3_buckets()

			# 8. Creating the topic Rules
			self.create_topic_rule()

			log.info("[END] created S3 buckets and Rules")
	...
	def remove(self, full_pipeline_flag=False):
		if not self.state:
		...
		############################# ADDED BY ANIRBAN#############################################
		if full_pipeline_flag:
			# 7. Deleting the required S3 buckets
			self.delete_s3_buckets()

			# 8. Deleting the topic Rules
			self.delete_topic_rule()

			log.info("[END] Deleted S3 buckets and Rules")
...
########################## Added by Anirban ###################################
	def update_deployment(self, full_pipeline_flag=False, reuse_lambda=False):
		...
		############################# ADDED BY ANIRBAN#############################################
		if full_pipeline_flag:
			# 7. Emptying the required S3 buckets
			self.delete_s3_buckets(empty_bucket_flag=True)

			log.info("[END] Emptied S3 buckets and Rules")
	...
	def reuse_lambdas(self, update_group_version=True):
		Lambda_def = self.group.get('Lambdas')
		...
		for l in self.group['Lambdas']:
			log.info("Reusing Lambda function '{0}'".format(l['name']))
		...
		################### UPDATED BY ANIRBAN  #######################################################################
			...
			lr = self._lambda.get_function(FunctionName=l['name'])['Configuration']

			self.state['Lambdas'].append(lr)
			_update_state(self.state)
			log.info("Lambda function '{0}' Reused".format(lr['FunctionName']))
		...
		log.info("Lambdas and function definition reused OK!")

	def create_topic_rule(self):
		if not self.group.get('Rules'):
			log.info("Topic Rules not defined. Moving on...")
			return

		if self.state and self.state.get('Rules'):
			log.warning("Previously created Rules exists. Remove before creating!")
			return

		log.debug("Creating topic rules...")
		topic_rules = []
		# Create the request syntax for creating topic rules from Yaml 
		for r in self.group['Rules']:
			# Convert from a simplified form
			rule_def = dict(ruleName=r.pop('ruleName'))
			rule_def['topicRulePayload'] = r.pop('topicRulePayload')
			topic_rules.append(rule_def)

		log.debug("Topic Rules list is ready:\n{0}".format(pretty(topic_rules)))
		for rule in topic_rules:
			self._iot.create_topic_rule(ruleName=rule['ruleName'], topicRulePayload=rule['topicRulePayload'])

		self.state['Rules'] = topic_rules
		_update_state(self.state)

		log.info("Topic Rules created OK!")

	def delete_topic_rule(self):
		if not (self.state and self.state.get('Rules')):
			log.info("There seem to be nothing to remove.")
			return

		for rule in self.state['Rules']:
			log.info("Deleting topic rule '{0}'".format(rule['ruleName']))
			self._iot.delete_topic_rule(ruleName=rule['ruleName'])

		self.state.pop('Rules')
		_update_state(self.state)
		log.info("Topic Rules deleted deleted OK!")

	def create_s3_buckets(self):
		if not self.group.get('ResultS3Buckets'):
			log.info("S3 buckets for results not defined. Moving on...")
			return

		if self.state and self.state.get('ResultS3Buckets'):
			print(self.state)
			log.warning("Previously created S3 buckets exists. Remove before creating!")
			return

		log.debug("Creating S3 buckets...")
		buckets = []
		# Create the request syntax for creating topic rules from Yaml 
		for r in self.group['ResultS3Buckets']:
			# Convert from a simplified form
			bucket_def = dict(Bucket=r.pop('Bucket'))
			bucket_def['CreateBucketConfiguration'] = r.pop('CreateBucketConfiguration')
			buckets.append(bucket_def)
		...
		log.info("S3 buckets created OK!")

	def delete_s3_buckets(self, empty_bucket_flag=False):
		if not (self.state and self.state.get('ResultS3Buckets')):
				log.info("There seem to be nothing to remove / empty.")
				return

		if empty_bucket_flag:
			s3 = resource('s3')
			for bucket in self.state['ResultS3Buckets']:
				log.info("Emptying S3 bucket '{0}'".format(bucket['Bucket']))
				current_bucket = s3.Bucket(bucket['Bucket'])
				current_bucket.objects.all().delete()
			log.info("S3 buckets emptied OK!")
		else:
			for bucket in self.state['ResultS3Buckets']:
				log.info("Deleting S3 bucket '{0}'".format(bucket['Bucket']))
				self._s3.delete_bucket(Bucket=bucket['Bucket'])

			self.state.pop('ResultS3Buckets')
			_update_state(self.state)
			log.info("S3 buckets deleted OK!")

	def reset_deployments(self):
		log.info("Resetting deployments")
		self._gg.reset_deployments(GroupId=self.state['Group']['Id'], Force=True)

	def pop_from_state(self, field):
